# =========================================================
# app.py — Eddies Questbook Edition — ULTIMATE (v4.3)
# KDP Mirror Margins + Dynamic Gutter + 300 DPI Preflight
# Sketch LRU Cache (per session) + 1-bit PNG compression
# GOD MODE: Visualizer + KDP Cover Helper
# Upload Counter: shows TOTAL uploaded images (not Streamlit list pagination)
# =========================================================
...
def _kdp_inside_gutter_in(pages: int) -> float:
    # KDP chart (inside/gutter depends on page count)
    if pages <= 150:
        return 0.375
    if pages <= 300:
        return 0.500
...
def safe_margins_for_page(
    ...
    is_odd = ((page_index_0 + 1) % 2 == 1)  # odd pages are right-hand pages in print
...
# =========================================================
# 4) SKETCH ENGINE (1-BIT) + SESSION LRU CACHE
# =========================================================
...
def _get_sketch_cached(img_bytes: bytes, target_w: int, target_h: int) -> bytes:
    """
    Per-session LRU that DOES NOT use img_bytes in cache-key (prevents huge key RAM).
    Key = (sha256(img), target_w, target_h)
    """
...
# =========================================================
# 7) GOD MODE: VISUALIZER & KDP HELPER
# =========================================================
def render_page_preview(
    ...
    """
    PIL preview with geometry overlays:
    - Trim (Red)
    - Safe (Green)
    - Header (Blue)
    - Card (Orange)
    """
    scale = 3.0
    w_px = int(pb.full_w * scale)
    h_px = int(pb.full_h * scale)
    ...
    sk_img.putalpha(170)
    img.paste(sk_img, (0, 0), sk_img)

    page_index_0 = page_num_1_based - 1
    sl, sr, stb = safe_margins_for_page(total_pages, kdp, page_index_0, pb)

    def px(v_pt: float) -> int:
        return int(v_pt * scale)

    # Trim rect (red) when bleed enabled
    if pb.bleed > 0:
        b = px(pb.bleed)
        draw.rectangle([b, b, w_px - b, h_px - b], outline=(255, 0, 0, 180), width=4)

    # Safe rect (green)
    safe_rect = [px(sl), px(stb), w_px - px(sr), h_px - px(stb)]
    draw.rectangle(safe_rect, outline=(0, 255, 0, 210), width=4)

    # Header & Card boxes (match overlay logic)
    header_h_pt = 0.75 * inch

    # Header: top inside safe area
    hx0 = px(sl)
    hy0 = px(stb)
    hx1 = w_px - px(sr)
    hy1 = hy0 + px(header_h_pt)
    draw.rectangle([hx0, hy0, hx1, hy1], fill=(0, 0, 255, 50), outline=(0, 0, 255, 200), width=3)

    # Mission selection same as build (content index maps to image index)
    # Here: use page index to approximate; for strict mapping use content_idx externally
    h_val = (start_hour + page_index_0) % 24
    seed = int(_stable_seed(name) ^ (page_index_0 << 1) ^ h_val) & 0x7FFFFFFF
    mission = qd.pick_mission_for_time(h_val, diff, seed)

    # Available height between header bottom and bottom safe
    content_h_pt = pb.full_h - 2 * stb
    avail_h_pt = content_h_pt - header_h_pt - 0.15 * inch

    pad_x = 0.18 * inch
    w_pt = pb.full_w - sl - sr
    sc = _autoscale_mission_text(mission, w_pt, 0, pad_x, avail_h_pt)
    card_h_pt = min(avail_h_pt, max(1.85 * inch, sc["needed"]))

    # Card: bottom anchored at bottom safe
    cx0 = px(sl)
    cx1 = w_px - px(sr)
    cy1 = h_px - px(stb)
    cy0 = cy1 - px(card_h_pt)
    draw.rectangle([cx0, cy0, cx1, cy1], fill=(255, 165, 0, 50), outline=(255, 165, 0, 220), width=3)
...
def render_kdp_helper(pages: int, paper_key: str):
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.info(f"**Spine Width**\n\n`{spine_in:.4f} in`\n\n(`{spine_mm:.2f} mm`)")
    with c2:
        st.write("**Calculator Settings (copy/paste)**")
        st.code(
            f"""Binding: Paperback
Interior: {paper_key}
Unit: Inches
...
def _uploads_signature(uploads_list) -> str:
    """
    Fast-ish signature without hashing full files:
    uses name + length + sha256(sample) where sample is first/last 2KB.
    """
    h = hashlib.sha256()
...
with st.container(border=True):
    ...
    # ✅ Upload Counter (THIS is what you asked for)
    n_uploads = len(uploads) if uploads else 0
    st.markdown(f"**📸 Hochgeladen:** `{n_uploads}` Bild(er)")
    st.caption("Hinweis: Die Anzeige im Uploader wie „Showing page 1 of …“ ist nur die Dateiliste-Paginierung von Streamlit – NICHT deine Buchseiten.")
...
if uploads and name:
    ...
    if n_uploads > 0:
        reuse_factor = content_pages / n_uploads
    else:
        reuse_factor = 0
    ...
    with st.expander("👁️ Layout-Röntgenblick (Visualizer)", expanded=False):
        st.caption("Check: Safe (grün) + Trim (rot) + Header (blau) + Card (orange). Gutter muss bei geraden/ungeraden Seiten spiegeln.")

        v_page = st.slider("Seite prüfen", 1, int(pages), 2)

        # Only content preview (intro=1, outro=last)
        if v_page == 1 or v_page == int(pages):
            st.info("Intro/Outro werden hier nicht gerendert. Nimm Seite 2 bis vorletzte Seite.")
        else:
            content_idx = v_page - 2  # page 2 -> content index 0
            up_idx = content_idx % max(1, n_uploads)
            up_obj = uploads[up_idx]
            b_data = up_obj.getvalue()

            diff = 1 if age <= 4 else 2 if age <= 6 else 3 if age <= 9 else 4
            prev_img = render_page_preview(
                page_num_1_based=v_page,
            ...
            st.image(
                prev_img,
                caption=f"Seite {v_page} ({'Rechts/Ungerade' if v_page % 2 != 0 else 'Links/Gerade'}) • Upload #{up_idx+1}/{n_uploads}",
                use_container_width=True,
            )

    # --- KDP Helper
    with st.expander("📏 KDP Template Daten (Cover Calculator)", expanded=False):
        render_kdp_helper(int(pages), paper)
...
if st.button("🚀 Buch generieren", disabled=not can_build):
    # Smart cache invalidation on uploads change
